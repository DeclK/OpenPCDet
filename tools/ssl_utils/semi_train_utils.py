...
def train_ssl_one_epoch(teacher_model, student_model, optimizer, labeled_loader, unlabeled_loader, epoch_id, lr_scheduler, accumulated_iter, ssl_cfg,
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        try:
            if unlabeled_loader_iter is not None:   # CHK MARK, if no unlabeled data
                ud_teacher_batch_dict, ud_student_batch_dict = next(unlabeled_loader_iter)
            else: ud_teacher_batch_dict, ud_student_batch_dict = None, None
...
def update_ema_variables(model, ema_model, alpha, global_step):
    ...
    for ema_param, param in zip(ema_model.parameters(), model.parameters()):
        ema_param.detach().mul_(alpha).add_(1 - alpha, param.data)  # CHK MARK, if use .data, loss.backward is extremely slow
        """
...
def update_ema_variables_with_fixed_momentum(model, ema_model, alpha):
    for ema_param, param in zip(ema_model.parameters(), model.parameters()):
        ema_param.detach().mul_(alpha).add_(1 - alpha, param.data)  # CHK MARK
        """
        if param.requires_grad:
            ema_param.data.mul_(alpha).add_(1 - alpha, param.data)
        else:
