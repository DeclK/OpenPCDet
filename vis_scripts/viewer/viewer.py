...
class Viewer:
    ...
    def set_r0_rect_mat(self, r0_rect):
    ...
    def show_3D(self, save_name=None, scale=1):
        ...
        self.vi.show(self.actors+self.actors_without_del,
                    camera=dict(pos=[0,0,100], viewup=[0, 1, 0], focalPoint=[0, 0, 0], viewAngle=45))
    ...
    def show_2D(self,box_color = (255,0,0),show_box_info=False,show_ids=True,points_colors=(0,0,255), save_name=None):
        """
            ...
            if boxes is None:
                ...
                for box_id in range(len(boxes)):
                    ...
                    if type(colors) is not str:
                        ...
                        color = np.array(mcolor.to_rgb(colors)) * 255
                        color = [color[2],color[1],color[0]]

                    pts_3d_cam = get_box_points(box)
                    pts_3d_cam = velo_to_cam(pts_3d_cam[:,0:3],self.cam_extrinsic_mat)

                    pts_3d_cam = np.matmul(pts_3d_cam, self.r0_rect.T)
                    img_pts = np.matmul(pts_3d_cam, self.cam_intrinsic_mat.T)  # (N, 4)
